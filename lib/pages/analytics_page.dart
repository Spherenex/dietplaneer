...
class _AnalyticsPageState extends State<AnalyticsPage>
  ...
  late List<Map<String, dynamic>> _mealRatings;
  late Map<String, dynamic> _nutrientBalance;
  late List<Map<String, dynamic>> _weightData;
  late List<Map<String, dynamic>> _insights;
  late Map<String, dynamic> _wellnessScore;
  ...
  void initState() {
    ...
    _nutrientBalance = {'protein': 30, 'carbs': 45, 'fat': 25};
    ...
    // Load personalized analytics based on user profile
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _loadPersonalizedAnalytics();
    });
  }

  // Load personalized analytics based on user profile
  void _loadPersonalizedAnalytics() {
    ...
    if (userProfile.name.isNotEmpty &&
      ...
      // Generate personalized analytics using the ML service
      final analytics = AnalyticsService.getRecommendedNutrition(userProfile);
      ...
      setState(() {
        ...
        _weightData = analytics['weightData'];
        _mealRatings = analytics['mealRatings'];
        _nutrientBalance = analytics['nutrientBalance'];
        _insights = analytics['insights'];
  ...
  Widget build(BuildContext context) {
    ...
    // When profile changes, reload analytics
    if (userProfile.name.isNotEmpty && !_dataLoaded) {
      _loadPersonalizedAnalytics();
    }
                  ...
                  ..._insights
                      .map(
                        (insight) => Column(
                          children: [
                            _buildInsightItem(
                              icon: _getIconFromString(insight['icon']),
                              title: insight['title'],
  ...
  Widget _buildGoalsTab(bool isMobile) {
    // Calculate current and target weight for goal card
    final lastWeight = _weightData.last['weight'];
    final firstWeight = _weightData.first['weight'];
    final targetWeight =
        lastWeight is double
            ? (lastWeight - 7.0).toStringAsFixed(1)
            : (lastWeight - 7).toString();

    // Calculate weight loss/gain
    final weightChange =
        (lastWeight is double && firstWeight is double)
            ? (lastWeight - firstWeight).abs().toStringAsFixed(1)
            : (lastWeight - firstWeight).abs().toString();

    // Determine if weight is trending in right direction for goal
    final userProfile = Provider.of<UserProfile>(context, listen: false);
    final isWeightTrendingRight =
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildGoalCard(
            title: 'Weight Goal',
            currentValue: '$lastWeight ${userProfile.weightUnit}',
            targetValue: '$targetWeight ${userProfile.weightUnit}',
            progress: isWeightTrendingRight ? 0.3 : 0.1,
            iconData: Icons.monitor_weight,
            subtitle:
                isWeightTrendingRight
                    ? '${userProfile.fitnessGoal == "Weight Loss" ? "Lost" : "Gained"} $weightChange ${userProfile.weightUnit}'
                    : 'Working towards your ${userProfile.fitnessGoal} goal',
          ...
          Wrap(
            ...
            children: [
              _buildSmallGoalCard(
                title: 'Protein',
                currentValue: '${_nutritionData['protein']['current']}g',
                targetValue: '${_nutritionData['protein']['goal']}g',
                progress:
                    _nutritionData['protein']['current'] /
                    _nutritionData['protein']['goal'],
                color: Colors.red,
              ),
              _buildSmallGoalCard(
                title: 'Carbs',
                currentValue: '${_nutritionData['carbs']['current']}g',
                targetValue: '${_nutritionData['carbs']['goal']}g',
                progress:
                    _nutritionData['carbs']['current'] /
                    _nutritionData['carbs']['goal'],
                color: Colors.amber,
              ),
              _buildSmallGoalCard(
                title: 'Fat',
                currentValue: '${_nutritionData['fat']['current']}g',
                targetValue: '${_nutritionData['fat']['goal']}g',
                progress:
                    _nutritionData['fat']['current'] /
                    _nutritionData['fat']['goal'],
                color: Colors.blue,
              ),
              _buildSmallGoalCard(
                title: 'Fiber',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  SizedBox(height: 550, child: _buildMealRatingChart()),
                  SizedBox(height: 8),
                  Center(
                    child: Wrap(
                      spacing: 16,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text(
                    'From $firstWeight to $lastWeight (${weightChange > 0 ? "+" : ""}${weightChange.toStringAsFixed(1)} ${Provider.of<UserProfile>(context, listen: false).weightUnit})',
                    style: TextStyle(fontSize: 14, color: Colors.grey[600]),
                  ),
                  SizedBox(height: 16),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceAround,
                    children: [
                      _buildScoreItem('Nutrition', _wellnessScore['nutrition']),
                      _buildScoreItem('Exercise', _wellnessScore['exercise']),
                      _buildScoreItem('Sleep', _wellnessScore['sleep']),
                      _buildScoreItem('Stress', _wellnessScore['stress']),
                    ],
                  ),
                ],
              ),
  ...
  // Helper method to convert string icon names to Icons
  ...
  Widget _buildWeightChart() {
    ...
    final List<double> weights =
        _weightData.map((item) => (item['weight'] as num).toDouble()).toList();
